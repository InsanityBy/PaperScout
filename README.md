...
- **Analyze**: Utilize LLMs (default supports [DeepSeek](https://www.deepseek.com)) to evaluate and score paper relevance (0-10 scale) based on customizable user interests, filter them by custom thresholds, extract tags, and translate titles and abstracts to Chinese.
...
- `configs.yaml`: System prompts, rate limits, chunk size settings, and the custom relevance threshold.
...
- **Completed States**
    - `200`: COMPLETED (Successfully completed the pipeline and uploaded)
    - `220`: IRRELEVANT (Filtered out by custom relevance threshold)
...
- **分析 (Analyze)**: 利用大语言模型（默认支持 [DeepSeek](https://www.deepseek.com)）根据自定义用户兴趣对论文进行 10 分制相关性打分，结合自定义阈值进行筛选，提取标签，并将标题和摘要翻译为中文。
...
- `configs.yaml`: 系统提示词、速率限制、分块设置，以及相关性过滤阈值。
...
- **已完成状态**
    ...
    - `220`: IRRELEVANT (未达到设定的相关性评分阈值，被过滤)
