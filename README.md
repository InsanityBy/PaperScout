...
- **Analyze**: Utilize LLMs (default supports [DeepSeek](https://www.deepseek.com)) to evaluate paper relevance based on customizable user interests, extract tags, and translate titles and abstracts to Chinese.
- **Upload**: Automatically format and upload relevant papers directly into specified Zotero collections.
...
- `configs.yaml`: System prompts, rate limits, and chunk size settings.
...
To run all stages sequentially (Fetch -> Parse -> Analyze -> Upload) for papers between 2024 and 2025:
...
# Only upload pending papers to Zotero
paper_scout -s 2024 -e 2025 --stage upload
```
...
- **`doi`** (Primary Key): The DOI of the paper (or a pseudo-DOI generated for missing ones).
- **`title` / `abstract`**: Original English title and abstract.
- **`title_cn` / `abstract_cn`**: Chinese translation generated by LLM.
- **`is_relevant`**: Boolean flag indicating if the paper matches your research interests by LLM.
- **`relevance_reason`**: The LLM's explanation for its relevance evaluation.
...
- **Pending States (Normal flow)**
    ...
    - `130`: PENDING_UPLOAD (Waiting to be uploaded to Zotero)
- **Completed States**
    - `200`: COMPLETED (Successfully uploaded)
    - `220`: IRRELEVANT (Filtered out by LLM)
...
- **分析 (Analyze)**: 利用大语言模型（默认支持 [DeepSeek](https://www.deepseek.com)）评估论文与自定义用户兴趣的相关性、提取标签，并将标题和摘要翻译为中文。
- **上传 (Upload)**: 自动格式化并将相关的论文直接上传至指定的 Zotero 库。
...
**运行完整工作流：**
按顺序执行 2024 至 2025 年的完整流程（获取 -> 解析 -> 分析 -> 上传）：
...
- **`is_relevant`**: 布尔值，标识 LLM 判断该论文是否与你的兴趣相关。
- **`relevance_reason`**: LLM 给出的相关性判定理由。
...
- **待处理状态（正常流程）**
    ...
    - `130`: PENDING_UPLOAD (等待上传至 Zotero)
- **已完成状态**
    - `200`: COMPLETED (成功完成全流程并已上传)
    - `220`: IRRELEVANT (经 LLM 判定为不相关，被过滤)
- **异常中间状态（系统会自动重试直到达到最大重试次数）**
    - `310`: PARSE_FAILED (解析失败)
    - `311`: DOI_INVALID (DOI 验证失败)
    - `320`: ANALYZE_FAILED (LLM 分析失败)
    - `330`: UPLOAD_FAILED (Zotero 上传失败)
- **异常终止状态（需要人工介入修改状态）**
